...
inline FilteredIterator<It, Output, Filter, positive>::FilteredIterator(It&& it)
    : _it(std::move(it)), _result(), _next(), _filter(), _got_next{true} {
  get_next();
}
template <class It, class Output, class Filter, bool positive>
inline FilteredIterator<It, Output, Filter, positive>::FilteredIterator(
    It&& it, Filter filter)
    : _it(std::move(it))
    , _result()
    , _next()
